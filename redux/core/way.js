...
const Way = Entity.way = function Way () {
...
_.extend(Way.prototype, {
  ...
  copy: function (deep, resolver) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      let oldid = this.nodes[i]
      let newid = replacements[oldid]
      if (!newid) {
        let child = resolver.entity(oldid).copy()
